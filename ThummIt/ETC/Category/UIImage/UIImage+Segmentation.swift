...
extension UIImage {
    ...
    @objc public func segmentation() -> CGImage? {
    ...
    @objc public func coarseSegmentation() -> CGImage? {
        
        let deeplab = Deeplab.init()
        //input size 513*513
        let pixBuf = self.pixelBuffer(width: 513, height: 513)
        guard let output = try? deeplab?.prediction(fromImageTensor__0: pixBuf!) else {
            return nil
        }
